...
let deleteUploadedPhotosBtn = document.querySelector(
  "#deleteUploadedPhotosBtn"
);
let imageToCrop = "";
  ...
  if (photosLs != null) {
    ...
    for (let i = 2; i < photosLs.length; i++) {
      ...
      newDot.addEventListener("click", () => {
        document.getElementById("slide").style.backgroundImage = `url(${
          photos[newDotId - 1]
        })`;
...
function checkPhotoBeforeUpload(sourceImg) {
  ...
  setTimeout(() => {
    sourceImgHeight =
      sourceImgElementsInDOM[
        sourceImgElementsInDOM.length - 1
    ...
    document
      ...
      .removeChild(sourceImgElementsInDOM[sourceImgElementsInDOM.length - 1]);
  }, 10);
  setTimeout(() => {
